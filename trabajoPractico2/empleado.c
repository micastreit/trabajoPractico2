...
printf("No hay mas lugar en el sistema.\n");
    ...
    while(!validarIdSector(nuevoEmpleado.sector,sectores,tamsec)){
...
{
    char descripcion[20];
    if(cargarDescripcionSector(unEmpleado.sector,sectores,tamsec,descripcion))
    {
...
if(lista!=NULL&&tam>0){
    ...
    scanf("%f",&lista[indice].sector);
    while(!validarIdSector(lista[indice].sector,sectores,tamsec)){
        printf("\nSector invalido. Reingrese id del sector:");
